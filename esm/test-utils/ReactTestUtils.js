...
import { ClassComponent, FunctionComponent, HostComponent, HostText } from "../react-shared/ReactWorkTags.js";
...
import * as DOMTopLevelEventTypes from "../events/DOMTopLevelEventTypes.js"; // for .act's return value

const {
  findDOMNode
} = ReactDOM; // Keep in sync with ReactDOMUnstableNativeDependencies.js
// and ReactDOM.js:

const [getInstanceFromNode,
/* eslint-disable no-unused-vars */
getNodeFromInstance, getFiberCurrentPropsFromNode, injectEventPluginsByName,
/* eslint-enable no-unused-vars */
eventNameDispatchConfigs, accumulateTwoPhaseDispatches, accumulateDirectDispatches, enqueueStateRestore, restoreStateIfNeeded, dispatchEvent, runEventsInBatch] = ReactDOM.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.Events;
...
const ReactTestUtils = {
    ...
    return ReactTestUtils.findAllInRenderedTree(root, function (inst) {
      if (ReactTestUtils.isDOMComponent(inst)) {
        ...
        if (!Array.isArray(classNames)) {
          invariant(classNames !== undefined, 'TestUtils.scryRenderedDOMComponentsWithClass expects a ' + 'className as a second argument.');
  ...
  mockComponent: function (module, mockTagName) {
    if (!hasWarnedAboutDeprecatedMockComponent) {
      ...
      lowPriorityWarning(false, 'ReactTestUtils.mockComponent() is deprecated. ' + 'Use shallow rendering or jest.mock() instead.\n\n' + 'See https://fb.me/test-utils-mock-component for more information.');
    }
  ...
  act(callback) {
    ...
    false) {
      if (result !== undefined) {
        ...
        warningWithoutStack(false, 'The callback passed to ReactTestUtils.act(...) function must not return anything.%s', addendum);
      }
    }
...
function makeSimulator(eventType) {
  return function (domNode, eventData) {
    invariant(!React.isValidElement(domNode), 'TestUtils.Simulate expected a DOM node as the first argument but received ' + 'a React element. Pass the DOM node you wish to simulate the event on instead. ' + 'Note that TestUtils.Simulate will not work if you are using shallow rendering.');
    invariant(!ReactTestUtils.isCompositeComponent(domNode), 'TestUtils.Simulate expected a DOM node as the first argument but received ' + 'a component instance. Pass the DOM node you wish to simulate the event on instead.');
    ...
    const event = new SyntheticEvent(dispatchConfig, targetInst, fakeNativeEvent, domNode); // Since we aren't using pooling, always persist the event. This will make
