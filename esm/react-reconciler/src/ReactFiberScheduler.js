...
import { NoEffect, PerformedWork, Placement, Update, Snapshot, PlacementAndUpdate, Deletion, ContentReset, Callback, DidCapture, Ref, Incomplete, HostEffectMask, Passive } from "../../react-shared/ReactSideEffectTags.js";
import { ClassComponent, HostComponent, ContextProvider, ForwardRef, FunctionComponent, HostPortal, HostRoot, MemoComponent, SimpleMemoComponent, SuspenseComponent, DehydratedSuspenseComponent } from "../../react-shared/ReactWorkTags.js";
import { enableSchedulerTracing, enableProfilerTimer, enableUserTimingAPI, replayFailedUnitOfWorkWithInvokeGuardedCallback, warnAboutDeprecatedLifecycles, enableSuspenseServerRenderer } from "../../react-shared/ReactFeatureFlags.js";
...
import { getStackByFiberInDevAndProd, phase as ReactCurrentFiberPhase, resetCurrentFiber, setCurrentFiber } from "./ReactCurrentFiber.js";
import { now, scheduleDeferredCallback, cancelDeferredCallback, shouldYield, prepareForCommit, resetAfterCommit, scheduleTimeout, cancelTimeout, noTimeout, schedulePassiveEffects, cancelPassiveEffects } from "./ReactFiberHostConfig.js";
import { markPendingPriorityLevel, markCommittedPriorityLevels, markSuspendedPriorityLevel, markPingedPriorityLevel, hasLowerPriorityWork, isPriorityLevelSuspended, findEarliestOutstandingPriorityLevel, didExpireAtExpirationTime } from "./ReactFiberPendingPriority.js";
...
import { NoWork, Sync, Never, msToExpirationTime, expirationTimeToMs, computeAsyncExpiration, computeInteractiveExpiration } from "./ReactFiberExpirationTime.js";
...
import { isContextProvider as isLegacyContextProvider, popTopLevelContextObject as popTopLevelLegacyContextObject, popContext as popLegacyContext } from "./ReactFiberContext.js";
...
import { recordCommitTime, startProfilerTimer, stopProfilerTimerIfRunningAndRecordDelta } from "./ReactProfilerTimer.js";
import { checkThatStackIsEmpty, resetStackAfterFatalErrorInDev } from "./ReactFiberStack.js";
import { beginWork } from "./ReactFiberBeginWork.js";
import { completeWork } from "./ReactFiberCompleteWork.js";
import { throwException, unwindWork, unwindInterruptedWork, createRootErrorUpdate, createClassErrorUpdate } from "./ReactFiberUnwindWork.js";
import { commitBeforeMutationLifeCycles, commitResetTextContent, commitPlacement, commitDeletion, commitWork, commitLifeCycles, commitAttachRef, commitDetachRef, commitPassiveHookEffects } from "./ReactFiberCommitWork.js";
...
let isWorking = false; // The next work in progress fiber that we're currently working on.

let nextUnitOfWork = null;
let nextRoot = null; // The time at which we're currently rendering work.

let nextRenderExpirationTime = NoWork;
let nextLatestAbsoluteTimeoutMs = -1;
let nextRenderDidError = false; // The next fiber with an effect that we're currently committing.
...
let legacyErrorBoundariesThatAlreadyFailed = null; // Used for performance tracking.
...
function resetChildExpirationTime(workInProgress, renderTime) {
  ...
  if (enableProfilerTimer && workInProgress.mode & ProfileMode) {
    ...
    let treeBaseDuration = workInProgress.selfBaseDuration; // When a fiber is cloned, its actualDuration is reset to 0.
  ...
  while (true) {
    ...
    if ((workInProgress.effectTag & Incomplete) === NoEffect) {
      ...
      (returnFiber.effectTag & Incomplete) === NoEffect) {
        ...
        const effectTag = workInProgress.effectTag; // Skip both NoWork and PerformedWork tags when creating the effect list.
      ...
      const next = unwindWork(workInProgress, nextRenderExpirationTime); // Because this fiber did not complete, don't reset its expiration time.
...
function performUnitOfWork(workInProgress) {
  ...
  // means that we don't need an additional field on the work in
  // progress.
  const current = workInProgress.alternate; // See if beginning this work spawns more work.
...
function renderRoot(root, isYieldy) {
  ...
  const expirationTime = root.nextExpirationTimeToWorkOn; // Check if we're starting from a fresh stack, or if we're resuming from
  ...
  invariant(rootWorkInProgress !== null, 'Finished root should have a work-in-progress. This error is likely ' + 'caused by a bug in React. Please file an issue.'); // `nextRoot` points to the in-progress root. A non-null value indicates
  // that we're in the middle of an async render. Set it to null to indicate
  // there's no more work to be done in the current batch.
  ...
  if (isYieldy && nextLatestAbsoluteTimeoutMs !== -1) {
    ...
    const suspendedExpirationTime = expirationTime;
    markSuspendedPriorityLevel(root, suspendedExpirationTime); // Find the earliest uncommitted expiration time in the tree, including
...
function computeExpirationForFiber(currentTime, fiber) {
  ...
  if (priorityLevel === UserBlockingPriority && (lowestPriorityPendingInteractiveExpirationTime === NoWork || expirationTime < lowestPriorityPendingInteractiveExpirationTime)) {
...
let currentSchedulerTime = currentRendererTime; // Use these to prevent an infinite loop of nested updates
...
function flushRoot(root, expirationTime) {
  invariant(!isRendering, 'work.commit(): Cannot commit while already rendering. This likely ' + 'means you attempted to commit from inside a lifecycle method.'); // Perform work on root as if the given expiration time is the current time.
...
function onUncaughtError(error) {
  invariant(nextFlushedRoot !== null, 'Should be working on a root. This error is likely caused by a bug in ' + 'React. Please file an issue.'); // Unschedule this root so we don't work on it again until there's
  // another update.
...
export { requestCurrentTime, computeExpirationForFiber, captureCommitPhaseError, onUncaughtError, renderDidSuspend, renderDidError, pingSuspendedRoot, retryTimedOutBoundary, markLegacyErrorBoundaryAsFailed, isAlreadyFailedLegacyErrorBoundary, scheduleWork, requestWork, flushRoot, batchedUpdates, unbatchedUpdates, flushSync, flushControlled, Scheduler_next as deferredUpdates, syncUpdates, interactiveUpdates, flushInteractiveUpdates, computeUniqueAsyncExpiration, flushPassiveEffects };