...
import { warnAboutDeprecatedLifecycles, enableSuspenseServerRenderer } from "../react-shared/ReactFeatureFlags.js";
import { REACT_FORWARD_REF_TYPE, REACT_FRAGMENT_TYPE, REACT_STRICT_MODE_TYPE, REACT_CONCURRENT_MODE_TYPE, REACT_SUSPENSE_TYPE, REACT_PORTAL_TYPE, REACT_PROFILER_TYPE, REACT_PROVIDER_TYPE, REACT_CONTEXT_TYPE, REACT_LAZY_TYPE, REACT_MEMO_TYPE } from "../react-shared/ReactSymbols.js";
import { emptyObject, processContext, validateContextBounds } from "./ReactPartialRendererContext.js";
import { allocThreadID, freeThreadID } from "./ReactThreadIDAllocator.js";
import { createMarkupForCustomAttribute, createMarkupForProperty, createMarkupForRoot } from "./DOMMarkupOperations.js";
import escapeTextForBrowser from "./escapeTextForBrowser.js";
import { prepareToUseHooks, finishHooks, Dispatcher, currentThreadID, setCurrentThreadID } from "./ReactPartialRendererHooks.js";
import { Namespaces, getIntrinsicNamespace, getChildNamespace } from "../shared/DOMNamespaces.js";
...
import { validateProperties as validateUnknownProperties } from "../shared/ReactDOMUnknownPropertyHook.js"; // Based on reading the React.Children implementation. TODO: type this somewhere?
...
if (__DEV__) {
  ...
  pushElementToDebugStack = function (element) {
    ...
    const stack = currentDebugStacks[currentDebugStacks.length - 1]; // Take the innermost executing frame (e.g. <Foo>),

    const frame = stack[stack.length - 1]; // and record that it has one more element associated with it.
  ...
  getCurrentServerStackImpl = function () {
    ...
    let stack = ''; // Go through every frame in the stack from the innermost one.

    for (let i = frames.length - 1; i >= 0; i--) {
      const frame = frames[i]; // Every frame might have more than one debug element stack entry associated with it.
...
function warnNoop(publicInstance, callerName) {
  if (__DEV__) {
    ...
    warningWithoutStack(false, '%s(...): Can only update a mounting component. ' + 'This usually means you called %s() outside componentWillMount() on the server. ' + 'This is a no-op.\n\nPlease check the code for the %s component.', callerName, callerName, componentName);
  ...
  function processChild(element, Component) {
        ...
        if (__DEV__) {
          if (warnAboutDeprecatedLifecycles && inst.componentWillMount.__suppressDeprecationWarning !== true) {
            ...
            if (!didWarnAboutDeprecatedWillMount[componentName]) {
              lowPriorityWarning(false, '%s: componentWillMount() is deprecated and will be ' + 'removed in the next major version. Read about the motivations ' + 'behind this change: ' + 'https://fb.me/react-async-component-lifecycle-hooks' + '\n\n' + 'As a temporary workaround, you can rename to ' + 'UNSAFE_componentWillMount instead.', componentName);
    ...
    if (typeof inst.getChildContext === 'function') {
      ...
      if (typeof childContextTypes === 'object') {
        ...
        warningWithoutStack(false, '%s.getChildContext(): childContextTypes must be defined in order to ' + 'use getChildContext().', getComponentName(Component) || 'Unknown');
      }
    }
        ...
        if (suspended || frame.childIndex >= frame.children.length) {
          ...
          if (frame.type === 'select') {
            ...
            if (suspended) {
              ...
              invariant(fallbackFrame, 'suspense fallback not found, something is broken');
              this.stack.push(fallbackFrame); // Skip flushing output since we're switching to the fallback
    ...
    if (typeof child === 'string' || typeof child === 'number') {
      ...
      if (nextChild === null || nextChild === false) {
        ...
        if (nextChild != null && nextChild.$$typeof != null) {
          ...
          invariant($$typeof !== REACT_PORTAL_TYPE, 'Portals are not currently supported by the server renderer. ' + 'Render them conditionally so that they only appear on the client render.'); // Catch-all to prevent an infinite loop if React.Children.toArray() supports some new type.
      ...
      if (typeof elementType === 'object' && elementType !== null) {
        switch (elementType.$$typeof) {
          ...
          case REACT_MEMO_TYPE:
            {
              const element = nextChild;
              let nextChildren = [React.createElement(elementType.type, Object.assign({
                ref: element.ref
              }, element.props))];
              const frame = {
                type: null,
  ...
  renderDOM(element, context, parentNamespace) {
    ...
    if (__DEV__) {
      if (namespace === Namespaces.html) {
        ...
        // allow <SVG> or <mATH>.
        warning(tag === element.type, '<%s /> is using incorrect casing. ' + 'Use PascalCase for React components, ' + 'or lowercase for HTML elements.', element.type);
      }
    }
